...
# Flag for Rewrd Normalization
REW_NORMALIZATION_BY_DIST = True

# Curriculums for learning
BASIC_CONTROL = 0
REDUCE_ENERGY_COST = 1
REACH_TARGET = 2
...
class Highway(gym.Env):
    ...
    """
    def __init__(self, sim_params, road_params, use_gui=True, curriculum = BASIC_CONTROL):
        ...
        self.ego_tcop = 0
        self.action = 0
        self.lvd = 0 # Leading vehicle distance
    ...
    def reset(self, sumo_ctrl=False):
        ...
        self.ego_tcop += init_energy_cost
    ...
    def step(self, action, action_info=None, sumo_ctrl=False):
        ...
        new_pos = self.positions[0, 0]
        self.dist_per_step = new_pos - prev_pos
    ...
    def reward_model(self, new_state, action, ego_collision=False, ego_near_collision=False, outside_road=False, dist_per_step = 0):
        """
        ...
        # Add penalty for collision/Near collision/outside road
        if self.curriculum >= BASIC_CONTROL:
            if outside_road or ego_near_collision or ego_collision:
                reward -= (self.outside_road_penalty)

            if action == CHANGE_LANE_LEFT or action == CHANGE_LANE_RIGHT:
                time_consumed = self.lat_control_duration / 3600 # hours
                dist_travelled = self.lane_width
            else:
                time_consumed = self.long_control_duration / 3600 # hours
                dist_travelled = dist_per_step
            
            if REW_NORMALIZATION_BY_DIST == True:
                reward -= ((time_consumed * 50) / dist_travelled) # driver cost - 50 euro / hr
            else:
                reward -= ((time_consumed * 50)) # driver cost - 50 euro / hr

        # Add penalty for energy cost
        if self.curriculum >= REDUCE_ENERGY_COST:
            if action == CHANGE_LANE_LEFT or action == CHANGE_LANE_RIGHT:
            ...
            if REW_NORMALIZATION_BY_DIST == True:
                reward -= ((electricity_consumed * 0.5) / dist_travelled) # electricity cost - 0.5 euro/kwh    
            else:
                reward -= ((electricity_consumed * 0.5))
       
        # Add reward for reaching destination
        if self.curriculum >= REACH_TARGET:
            if new_state[4][0] == "exit":   
                reward += (self.completion_reward)

        # Compute evaluation parameters
        self.compute_eval_params(new_state, action)
        return reward

    def compute_eval_params(self, new_state, action):
        """
        Compute evaluation metrics (used for validation)
        ...
        if action == CHANGE_LANE_LEFT or action == CHANGE_LANE_RIGHT:
            energy_consumed = self.lat_controller.energy_consumed #kwh
            time_consumed = (self.lat_control_duration / 3600) #hr
        else:
            energy_consumed = self.long_controller.energy_consumed #kwh
            time_consumed = (self.long_control_duration / 3600) #hr
            
        self.ego_energy_cost += (energy_consumed * 0.5)

        self.ego_driver_cost += (time_consumed * 50)
        self.ego_tcop += ((energy_consumed * 0.5) + (time_consumed * 50))
