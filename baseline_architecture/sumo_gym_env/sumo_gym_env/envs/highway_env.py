...
ROAD_ID = 80 # Edge name
 ...
 # A flag to indicate whether lead vehicle distance should be included in states or not
LEAD_VEH_DIST_STATE = 1
...
class Highway(gym.Env):
    ...
    def __init__(self, sim_params, road_params, use_gui=True):
        ...
        if LEAD_VEH_DIST_STATE:
            self.nb_ego_states = 6
        else:
            self.nb_ego_states = 5
        ...
        # Define an action space ranging from 0 to 9
        ...
        if sim_params['remove_sumo_warnings']:
            traci.start([sumo_binary, "-c", self.road.road_path + self.road.name + ".sumocfg", "--start", "--no-warnings"])
        else:
    ...
    def reset(self, sumo_ctrl=False):
        ...
        traci.vehicle.setAcceleration(self.ego_id, 0, self.init_steps)
    ...
    def step(self, action, action_info=None, sumo_ctrl=False):
        ...
        if ((self.lanes[0] == 0) and (lat_action == -1)) or ((self.lanes[0] == (self.nb_lanes - 1)) and (lat_action == 1)):
            ...
            traci.vehicle.changeLaneRelative(self.ego_id, lat_action, 1e15)  # 1e15 is durarion: the lane will be chosen for the given amount of time (in s). 
        traci.simulationStep()
        ...
        if collision:
            ...
            info.append([traci.vehicle.getSpeed(veh) for veh in colliding_ids])
            if self.step_ == 0:
    ...
    def reward_model(self, new_state, action, ego_collision=False, ego_near_collision=False, outside_road=False):
        ...
        """        
        # Reward propotional to speed
        reward = new_state[1][0, 0] / self.max_allowed_ego_speed
        # Lane change penalty
        ...
        # Reward for episode completion (ego vehicle reached exit edge) based on time steps it took
